...
export default function NewExpenseDialog(props: Props): JSX.Element {
  ...
  function Actions({
    isSubmitting,
    submitForm,
  }: { isSubmitting: boolean, submitForm: () => Promise<unknown> }): JSX.Element {
    const CancelButton = () => (
    ...
    const NextButton = () => (
      <Button
        color="primary"
        onClick={ nextStep }
        disabled={ !stepState.canNextStep }
      >
        Next
      </Button>
    );
      ...
      { ({
        ...
        <form onSubmit={ handleSubmit }>
          <Dialog open={ props.isOpen } maxWidth="sm">
            ...
            <DialogActions>
              <Actions isSubmitting={ isSubmitting } submitForm={ submitForm } />
            </DialogActions>
          </Dialog>
        </form>
      ) }
