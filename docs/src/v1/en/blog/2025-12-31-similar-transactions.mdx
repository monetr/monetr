...
head:
  ...
  - - meta
    - property: og:title
      content: "How monetr's similar transactions work"
...
<BlogMeme src='/v1/en/blog/2025-12-31-similar-transactions/dumbstuff.png' alt='Dumb Stuff'/>

:::info
This blog post contains information that is loosely understood and probably **wrong**. Do not mistake this blog post and
the achievement of the desired results as proof that I know what I'm doing, or that the things discussed here are 100%
factual. This blog post covers what I have done based on my loose understanding and assumptions only. **I am simply
making mistakes in public so that people can point them out sooner.**
:::
...
```text /POS/ /DEBIT/ /CARIBOU/ /COFFE/ /NORTH/ /BRANCH/ title="Original Name + Desired Tokens"
POS DEBIT-DC 1234 TST* CARIBOU COFFE NORTH BRANCH MN
```
...
```text /pos/ /debit/ /caribou/ /coffee/ /north/ /branch/ title="Original Name + Desired Tokens + Transformations"
pos debit caribou coffee north branch
```

Overall this is what is happening:
1. Split the string by non-word characters.
2. Throw out any words that are purely numeric or contain no vowels, these are usually pieces of information that are
   not valuable anyway.
...
document. More precisely, the number of unique non-useless tokens in the document. Tokens or words that were thrown out
in the tokenization step are not considered in this count.
:::

Once all of the documents have been added, the "idf" can be calculated. This is done using the following formula.

$$
\text{idf}_{word} = \log\left(\frac{\text{number of documents}}{\text{count}_{word} + 1}\right)
$$

:::info
**Note**: This isn't quite the same weighting scheme as __inverse document frequency__ or __inverse document frequency
smoothed__. I've simply translated what I have in code and its very possible (read likely) that I implemented this part
wrong.
:::
...
```go title="tf-idf Vector Before Normalization"
vector := []float32{
  0.000000, // acctverify
  0.000000, // ach
  0.000000, // adjustment
...
```go title="tf-idf Vector After Normalization"
vector := []float32{
  0.000000, // acctverify
  0.000000, // ach
  0.000000, // adjustment
...
```json title="Tokens From The Centroid Transaction"
[
  {
    "word": "Caribou",
    "sanitized": "caribou",
...
```json title="Tokens In The Top 75%"
[
    {
      "word": "Caribou",
      "sanitized": "caribou",
...
```text title="Transaction Cluster Name"
Caribou Coffee
```
...
3. Right now this entire process is re-performed from scratch every time there are transaction changes (file upload or
   ...
   - It is worth noting here that the DBSCAN portion of this is _the most expensive part of this by far_. Calculating the
     ...
     ![Similar Transaction Trace](/v1/en/blog/2025-12-31-similar-transactions/similar-trace.png)
...
:::warning
At the time of writing this blog post, some of the functionality and behaviors discussed in this blog post were made as
part of writing it (turns out looking at your code to write about what it's doing makes you realize how bad your code
is). So some of the improvements made as part of writing this blog post have not been released (at the time of writing
this). These improvements will be available in the next update.
:::
