...
:::info
`CMAKE_PRESET` is ignored for some make targets, such as `release` and `test`. As each of those targets have a specific
preset that they are expected to use.
:::

Here is a sample of a user presets file with a few customized options:

```json title="CMakeUserPresets.json"
{
  "version": 5,
...
- `MONETR_KMS_PROVIDER`: As configured, enables a local instance of [Hashicorp Vault](https://www.vaultproject.io/) as
  ...
  `aws`, `openbao` or `vault`. **Note:** The AWS setting runs a local KMS service compatible with AWS KMS's API.
- `MONETR_STORAGE_PROVIDER`: A RustFS instance will be run as part of the local development stack. Valid values are `s3`
...
sudo ./build/tools/bin/mkcert -uninstall
# Reinstall the certificates without root
./build/tools/bin/mkcert -install
```
...
:::info
If you want to disable hot reloading of the Go code, you can include `DISABLE_GO_RELOAD=true` in your env variables when
...
- [RustFS](https://github.com/rustfs/rustfs) As an S3 storage backend. The console is accessible via
...
- [OpenBao](https://openbao.org/) An open-source fork of Vault that provides a self-hostable secret management and
  encryption service. Available at `https://openbao.monetr.local` if you have OpenBao enabled in your user preset.
...
![IntellJ IDEA Configuration](/v1/en/documentation/development/assets/IntellJ_IDEA_Go_Debugging.png)
...
There are additional environment variables that specifically manipulate how tests run. For example, all controller tests
will use a specific timestamp by default. But if there is a problem, or you want to force a specific timestamp to be
used in the test by default you can include the following environment variable in this format:

```ini title="Test Environment Variables"
MONETR_TIMESTAMP="2006-01-02 15:04:05.999999999 -0700 MST"
```

This will set the tests mock clock to be in the specified timestamp when the test runs. If the test fails it will also
print its own timestamp so that tests using random timestamps can have their bug reproduced. This is not widely
available throughout the entire test suite at this time, but it will be in the future.

In addition, accounts that are seeded using the controller fixtures package can have their timezone forced in a similar
way.

```ini title="Test Environment Variables"
MONETR_TIMEZONE="America/Chicago"
```

This will force the timezone of any accounts created to be in the timezone specified by the environment variable. By
default a random timezone is used. This is also printed at the end of the test if the test fails. This is not yet
implemented in the entire test suite but moving forward more tests will support these environment variables and more in
order to make reproducing issues easier, while still keeping the test suite somewhat random.

### Frontend

An alternative to work on only the frontend components of monetr is to run a "lite" development environment. The lite
environment only runs the frontend dev server and instead of requiring an active API server and additional services to
be running locally, you can instead point the frontend at a desired API server.
...
:::warning
Be extremely conscious about what actions you perform in the lite development environment, if you have your development
environment pointed at the production instance for example; any actions you take locally will affect your real account.
Any data modified or deleted will not be recoverable.
:::

### Documentation

monetr's documentation is built using Rspress, it can be developed locally using the following command:

```shell title="Shell"
make develop-docs
```

This will setup everything the documentation site needs and run the Rspress development server locally, you can now make
...
This will compile the documentation and output to `$PWD/build/docs/out`.

If you want you can then preview the built site using `npx http-server $PWD/build/docs/out -c-1`.
