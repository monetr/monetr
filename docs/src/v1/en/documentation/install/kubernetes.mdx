...
```yaml title="configmap.yaml" copy
kind: ConfigMap
apiVersion: v1
metadata:
  name: monetr-config
...
```yaml title="deployment.yaml" copy
apiVersion: apps/v1
kind: Deployment
metadata:
  name: monetr
...
```yaml title="authentication.yaml" copy
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
