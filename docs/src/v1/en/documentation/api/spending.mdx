...
Spending objects are the envelopes. Each one sits on a [bank account](./bank_accounts#the-bank-account-object), has a
target amount, and gets filled up a little at a time by a [funding schedule](./funding_schedules). When you spend money
...
Returns an array of transaction objects, same shape as [List transactions](./transactions#list-transactions).
...
And it can't be in the past, which is stricter than it sounds: midnight _today_ has already happened, so the earliest
...
[transfer](#transfer-between-spending-objects), not by writing amounts directly.
