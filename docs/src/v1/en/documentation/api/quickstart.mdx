...
Six calls, start to finish. By the end you'll have a bank account monetr is tracking, a paycheck schedule filling an
envelope, and a transaction spent out of that envelope, all created over the API.

Everything here uses a **manual link**, which is the kind you maintain yourself. Plaid links are created through a
browser handshake an API key can't drive, so a manual link is the only way to build an account from scratch over the
API. It's also the honest way to try things without connecting a real bank.
...
[`fundingScheduleId`](./spending#create-a-spending-object) is required and has to already be on the same bank account.
...
[Timestamps](./overview#timestamps) for why.
...
else in this API. Read [Money](./overview#money) once before you go further.
...
That `ruleset` says every other Friday. It's an [RFC 5545](https://datatracker.ietf.org/doc/html/rfc5545) recurrence
rule, and monetr is fussy about them in ways that aren't obvious, so if you get `"Ruleset must be valid"` back go read
[Recurrence Rules](./recurrence_rules). The [cookbook](./recurrence_rules#cookbook) has a copy-paste rule for every
...
The response carries the transaction, the account's recalculated balances, and the envelope that got debited, so you
don't need a follow up call. The [full response is described here](./transactions#create-a-transaction).
...
- [List transactions](./transactions#list-transactions) to read the feed back, and [Update a
  transaction](./transactions#update-a-transaction) to move one between envelopes.
- [Transfer between spending objects](./spending#transfer-between-spending-objects) when you need to move money you've
  already allocated.
- [Get a forecast](./forecast#get-a-forecast) to project this forward, and [Estimate the cost of a new spending
  object](./forecast#estimate-the-cost-of-a-new-spending-object) to price something before you commit to it.
