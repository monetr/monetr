...
:::warning
It is always recommended to make a backup of monetr's data before performing a migration like this. While this migration
should be non-destructive up until the final steps, it is always possible that the dry run succeeds and the actual
migration may fail. While monetr does try to perform the migration atomically such that there is no data loss even in
the event of a failure, it is always better to be safe and have a backup just in case.
:::
...
every secret currently stored on your monetr instance. Depending on your KMS provider this may incur additional charges.
...
location for monetr or you will need to pass `-c ${path to your config}` to monetr before the `admin` command. For
example: `monetr -c ${path to your config} admin ...`.
:::
