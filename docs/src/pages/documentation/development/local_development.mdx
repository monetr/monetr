...
<Callout type="info">
  `CMAKE_PRESET` is ignored for some make targets, such as `release`, `test` and `deploy`. As each of those targets have
...
will need to create a `CMakeUserPresets.json` file with a preset that has the following cache variables:
...
When running `make develop` CMake will copy and expose those two files such that the monetr API can access them inside
...
- [mailhog](https://github.com/mailhog/MailHog) An SMTP server that allows emails to be sent without really sending
