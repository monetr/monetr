...
Plaid Links are supporting resources behind the normal [Link](links) resource. They provide a way to retrieve real
...
Under the hood, monetr is making a call to [Create Link Token](https://plaid.com/docs/api/tokens/#linktokencreate).
...
```shell filename="Example New Link Token Request"
curl --request GET \
...
```json filename="500 Internal Server Error"
{
...
<Callout type="info">
  ...
  [![GitHub issue/pull request detail](https://img.shields.io/github/issues/detail/state/monetr/monetr/858?label=%23858%20-%20chore%3A%20Remove%20success%20field%20and%20any%20references%20from%20the%20%2Fapi%2Fplaid%2Ftoken%2Fcallback%20response.&logo=github)](https://github.com/monetr/monetr/issues/858)
</Callout>

### Plaid Token Callback Examples

```shell filename="Example Plaid Token Callback Request"
curl --request POST \
...
```shell filename="Example Update Link Request"
curl --request PUT \
...
```shell filename="Example manual resync request"
curl --request POST \
