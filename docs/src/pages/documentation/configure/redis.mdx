...
| `database` | Number   | `0`         | Database number that you want monetr to use for caching. Defaults to `0`.                                       |
| `username` | String   |             | Username used to authenticate to the Redis-compatible server, defaults to an empty string.                      |
| `password` | String   |             | Password used to authenticate to the Redis-compatible server, defaults to an empty string.                      |

<Callout type="info">
  monetr does not support TLS for this cache server at this time. Sensitive information is never cached on this server
  and the use of the cache is purely for performance.
</Callout>
...
| `MONETR_REDIS_DATABASE` | `redis.database`  |
| `MONETR_REDIS_USERNAME` | `redis.username`  |
| `MONETR_REDIS_PASSWORD` | `redis.password`  |
