...
To being, we represent our vector above as $\mathbf{x}$:
...
From this we take all of the words that appear in the centeroid transaction and sort them by their value. The most
...
the transaction). We then take the the top 75% of ranked words within this list and we sort them by the order they
appeared in the transaction (because the order of the words is not represented by their value necessarily).
...
This ends up being a pretty reasonable way to derive a merchant name from a transaction without the data being santized
...
3. Right now this entire process is re-performed from scratch every time there are transaction changes (file upload or
   ...
   - It is worth noting here that the DBSCAN portion of this is _the most expensive part of this by far_. Calculating the
     ...
     essentially negligable compared to how long the DBSCAN can take. This can be seen in the screenshot below which is
     ...
     As you can see, even the innefficient N+1 query and the tf-idf code is just insignificant compared to the time it
