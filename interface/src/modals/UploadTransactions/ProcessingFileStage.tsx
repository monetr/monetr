      ...
      <div className='flex flex-col gap-2 h-full'>
        ...
        <div className='flex gap-2 items-center border rounded-md w-full p-2 border-dark-monetr-border'>
          ...
          <div className='flex flex-col py-1 w-full'>
            <MSpan size='lg'>Processing: { data?.status || 'Pending' }</MSpan>
            <div className='w-full bg-gray-200 rounded-full h-1.5 my-2 dark:bg-gray-700 relative'>
              <div className='absolute top-0 bg-blue-600 h-1.5 rounded-full dark:bg-blue-600' style={ { width: '50%' } }></div>
            </div>
          </div>
        </div>
      </div>
...
function useTransactionUploadProgress(
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(`${protocol}://${ location.host }/api/bank_accounts/${ bankAccountId }/transactions/uploads/${ transactionUploadId }/progress`);
    socket.onopen = () => {
    ...
    socket.onmessage = event => {
      ...
      const queryKey = [`/bank_accounts/${ bankAccountId }/transactions/uploads/${ transactionUploadId }`];
  ...
  return useQuery<Partial<TransactionUpload>, unknown, TransactionUpload>(
    [`/bank_accounts/${ bankAccountId }/transactions/uploads/${ transactionUploadId }`],
    {
