...
export type PatchLinkRequest =
  Pick<Link, 'linkId'> &
  Partial<Pick<Link, 'institutionName' | 'description'>>;

export function usePatchLink(): (_: PatchLinkRequest) => Promise<Link> {
  const queryClient = useQueryClient();

  async function patchLink(
    { linkId, ...patch }: PatchLinkRequest,
  ): Promise<Link> {
    return request()
  ...
  const mutation = useMutation({
    ...
    onSuccess: (updated: Link) => Promise.all([
      queryClient.setQueryData(
        ['/links'],
        (previous: Array<Partial<Link>>) => (previous ?? [])
