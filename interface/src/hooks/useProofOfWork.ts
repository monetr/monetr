...
// Treat a challenge as stale a few seconds before its ttl to cover the request
// round trips. Absolute clock skew is irrelevant: each side only ever compares
// its own timestamps.
const STALE_MARGIN_MS = 5_000;

/**
 * useProofOfWork pre-solves a challenge on mount so the solution is ready by the
 * time the user submits (getSolution returns it, or null when disabled). It
 * solves once, never in the background; if the user idles past the ttl,
 * getSolution fetches and solves a fresh one then. A challenge is single use and
 * is consumed even on a failed submit, so call reset() after a failure.
 *
...
export function useProofOfWork(purpose: PowPurpose, enabled: boolean): UseProofOfWork {
  ...
  // When the current challenge goes stale (epoch ms); Infinity while a fetch is
  // in flight so we never restart a not-yet-solved one.
  ...
  const getSolution = useCallback((): Promise<PowSolution | null> => {
    ...
    // Fetch fresh if we have none or the pre-solved one went stale while idle.
    // This and reset are the only places we start solving (never in background).
