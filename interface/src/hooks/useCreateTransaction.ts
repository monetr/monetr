...
export function useCreateTransaction(): (_: CreateTransactionRequest) => Promise<CreateTransactionResponse> {
  ...
  const { mutateAsync } = useMutation({
    ...
    onSuccess: ({ transaction, balance, spending }: CreateTransactionResponse) => Promise.all([
      queryClient.setQueryData(
        [`/bank_accounts/${transaction.bankAccountId}/spending`],
        (previous: Array<Partial<Spending>>) => previous
          .map(item => spending?.spendingId == item.spendingId ? spending : item),
      ),
      spending != null && queryClient.setQueryData(
