...
func NewMigrationsManager(ctx context.Context, log *slog.Logger, db *bun.DB) (*MonetrMigrationsManager, error) {
	...
	if err := ensureSchemaTable(ctx, log, newBunExecutor(conn)); err != nil {
		return nil, err
	}
