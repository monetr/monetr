...
type KeyManagement struct {
	...
	// OpenBao provides configuration for using OpenBao's Transit API for
	// encrypting and decrypting secrets.
	OpenBao OpenBaoTransit `yaml:"openbao"`
}
...
type OpenBaoTransit struct {
	// The name of the OpenBao Transit key to be used for encryption and
	// decryption. This value can be changed but the old key must always be
	// accessible to allow old secrets to be decrypted. But new secrets will be
	// encrypted using the new key. This value cannot be left blank.
	KeyID string `json:"keyID"`
	// AuthMethod tells monetr how to authenticate OpenBao, potential values are:
	// - `token`: Use a token to authenticate to OpenBao.
	// - `kubernetes`: Use the container's Kubernetes Service Account Token to
	...
	// automatically refreshed using the contents of this file.
	TokenFile string `yaml:"tokenFile"`
	// OpenBao URL to make requests to.
	Endpoint string `yaml:"endpoint"`
	// Role is required if you are authenticating via `userpass` or `kubernetes`.
	// This role is sent with the authentication request to specify what role the
	// client will have.
	...
	// Password is required when you are authenticating via `userpass`.
	Password string `yaml:"password"`
	// TLSCertificatePath can be specified if your OpenBao server is using a
	// custom certificate authority. This path will also be monitored for changes
	// such that certificates can be rotated without downtime.
	TLSCertificatePath string `yaml:"tlsCertificatePath"`
	// Specify the path to the TLS key for OpenBao.
	TLSKeyPath string `yaml:"tlsKeyPath"`
	// Specify the path to the certificate authority that OpenBao is using.
	TLSCAPath string `yaml:"tlsCAPath"`
	// Ignores any TLS issues.
	InsecureSkipVerify bool `yaml:"insecureSkipVerify"`
}
