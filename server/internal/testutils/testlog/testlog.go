// Package testlog provides slog helpers for tests. It lives off in its own
// sub-package (rather than testutils proper) so that code which testutils
// itself depends on (such as server/migrations) can still pull in test
// loggers without forming an import cycle.
package testlog
