...
func TestCreateFundingSchedule(t *testing.T) {
	t.Run("happy path", func(t *testing.T) {
		...
		input := map[string]any{
			...
			"nextRecurrence": util.Midnight(time.Now(), timezone).UTC().Format(time.RFC3339),
		}

		var result models.FundingSchedule

		issues := schema.CreateFundingSchedule.Parse(input, &result, zog.WithCtxValue("timezone", timezone))
	...
	t.Run("excludeWeekends", func(t *testing.T) {
		timezone := testutils.Must(t, time.LoadLocation, "America/Chicago")
		input := map[string]any{
			"name":            "Test",
			"description":     "Foobar",
