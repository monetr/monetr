...
func (c *Controller) postSpendingTransfer(ctx *echo.Context) error {
	...
	if transfer.FromSpendingId != nil {
		fromExpense, err := repo.GetSpendingById(
			c.getContext(ctx),
			bankAccountId,
			*transfer.FromSpendingId,
		)
		if err != nil {
		...
		fundingSchedule, err = repo.GetFundingSchedule(
			c.getContext(ctx),
			bankAccountId,
			fromExpense.FundingScheduleId,
		)
		if err != nil {
			return c.wrapPgError(
				ctx,
				err,
				"Failed to retrieve funding schedule for source goal/expense",
			)
		}

		fromExpense.CurrentAmount -= transfer.Amount
		fromExpense.CalculateNextContribution(
			c.getContext(ctx),
			timezone,
			fundingSchedule,
			c.Clock.Now(),
		...
		if fundingSchedule == nil || fundingSchedule.FundingScheduleId != toExpense.FundingScheduleId {
			...
			if err != nil {
				return c.wrapPgError(
					ctx,
					err,
					"Failed to retrieve funding schedule for destination goal/expense",
				)
			}
		}

		toExpense.CurrentAmount += transfer.Amount
		toExpense.CalculateNextContribution(
			c.getContext(ctx),
			timezone,
			fundingSchedule,
			c.Clock.Now(),
...
func (c *Controller) patchSpending(ctx *echo.Context) error {
	...
	// Only allow auto creating transactions for manual links, this is also
	// enforced to be only available on expenses via the schema.
	if updatedSpending.AutoCreateTransaction {
		isManual, err := repo.GetLinkIsManualByBankAccountId(
			c.getContext(ctx),
			bankAccountId,
