...
func TestPostTransactionImport(t *testing.T) {
	...
	t.Run("with a valid api key", func(t *testing.T) {
		// The import endpoints accept API key authentication. Uploading a CSV with a
		// valid API key that belongs to the same account must succeed exactly the
		// same way a session token does.
		app, e := NewTestApplication(t)
		var token string
		var bank BankAccount
		...
		// The API key must belong to the same account that owns the bank account, so
		// it is created with the same session token used to seed the account.
	...
	t.Run("with an invalid api key", func(t *testing.T) {
		...
		// the upload is ever processed, so Store is never called.
		app, e := NewTestApplication(t)
...
func TestGetTransactionImport(t *testing.T) {
	...
	t.Run("with a valid api key", func(t *testing.T) {
		// Reading a transaction import accepts API key authentication. A valid API
		// key that belongs to the owning account must be able to read the import.
		app, e := NewTestApplication(t)
		user, password := fixtures.GivenIHaveABasicAccount(t, app.Clock)
		link := fixtures.GivenIHaveAManualLink(t, app.Clock, user)
		bank := fixtures.GivenIHaveABankAccount(t, app.Clock, &link, DepositoryBankAccountType, CheckingBankAccountSubType)
		...
		// The API key must belong to the same account that owns the import, so it is
		// created with the same session token used to seed the account.
		apiKeyId, apiKeySecret := GivenIHaveAnApiKey(t, e, token)
		...
		var transactionImportId ID[TransactionImport]
		{ // Create the transaction import, seeded with the session token.
			response := e.POST("/api/bank_accounts/{bankAccountId}/transactions/import").
				WithPath("bankAccountId", bank.BankAccountId).
				WithMultipart().
				WithFileBytes("data", "transactions.csv", []byte("Date,Description,Amount\n2026-01-15,COFFEE,-4.50\n")).
	...
	t.Run("with an invalid api key", func(t *testing.T) {
		// A well-formed but non-existent API key must be rejected with a 401 before
		// the import lookup ever happens.
		_, e := NewTestApplication(t)
...
func TestPatchTransactionImport(t *testing.T) {
	...
	t.Run("with a valid api key", func(t *testing.T) {
		// The mapping -> pending-preview transition accepts API key authentication.
		// A valid API key that belongs to the owning account must be able to patch
		// the import and kick off the preview job exactly like a session token does.
		app, e := NewTestApplication(t)
		var token string
		var bank BankAccount
		...
		var mappingId ID[TransactionImportMapping]
		{ // Create a mapping whose headers line up with the uploaded CSV.
			response := e.POST("/api/mappings").
				WithCookie(TestCookieName, token).
				WithJSON(map[string]any{
					"mapping": map[string]any{
		...
		// The PATCH wraps the enqueue in the same db transaction as the status
		// update, so WithTransaction lands first and the EnqueueAt call lands on the
		// transactional queue.
		app.Queue.EXPECT().
			WithTransaction(
				gomock.Any(),
