...
func (r *RemoveLinkJob) Run(ctx context.Context) error {
	...
	if len(bankAccountIds) > 0 {
...

		{
			result, err := r.db.ModelContext(span.Context(), &models.PlaidTransaction{}).
				Table("transactions").
				Where(`"plaid_transaction"."plaid_transaction_id" IN ("transactions"."plaid_transaction_id", "transactions"."pending_plaid_transaction_id")`).
				Where(`"transactions"."account_id" = "plaid_transaction"."account_id"`).
				Where(`"plaid_transaction"."account_id" = ?`, accountId).
				WhereIn(`"transactions"."bank_account_id" IN (?)`, bankAccountIds).
				Delete()
			if err != nil {
...

		{
			result, err := r.db.ModelContext(span.Context(), &models.TellerTransaction{}).
				Table("transactions").
				Where(`"teller_transaction"."teller_transaction_id" = "transactions"."teller_transaction_id"`).
				Where(`"transactions"."account_id" = "teller_transaction"."account_id"`).
...

		{
			result, err := r.db.ModelContext(span.Context(), &models.TellerSync{}).
				Table("bank_accounts").
				Where(`"teller_sync"."teller_bank_account_id" = "bank_accounts"."teller_bank_account_id"`).
				Where(`"teller_sync"."account_id" = "bank_accounts"."account_id"`).
				Where(`"teller_sync"."account_id" = ?`, accountId).
				WhereIn(`"bank_accounts"."bank_account_id" IN (?)`, bankAccountIds).
				Delete()
			if err != nil {
...

		{
			result, err := r.db.ModelContext(span.Context(), &models.PlaidBankAccount{}).
				Table("bank_accounts").
				Where(`"plaid_bank_account"."plaid_bank_account_id" = "bank_accounts"."plaid_bank_account_id"`).
				Where(`"plaid_bank_account"."account_id" = "bank_accounts"."account_id"`).
...

		{
			result, err := r.db.ModelContext(span.Context(), &models.TellerBankAccount{}).
				Table("bank_accounts").
				Where(`"teller_bank_account"."teller_bank_account_id" = "bank_accounts"."teller_bank_account_id"`).
				Where(`"teller_bank_account"."account_id" = "bank_accounts"."account_id"`).
...

	{
		result, err := r.db.ModelContext(span.Context(), &models.PlaidSync{}).
			Table("links").
			Where(`"plaid_sync"."plaid_link_id" = "links"."plaid_link_id"`).
			Where(`"plaid_sync"."account_id" = "links"."account_id"`).
			Where(`"plaid_sync"."account_id" = ?`, accountId).
			Where(`"links"."link_id" = ?`, linkId).
			Delete()
		if err != nil {
