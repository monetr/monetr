...
func (e *Spending) CalculateNextContribution(
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse account's timezone")
	}
	...
	if eventsBeforeSecond > 0 {
		...
		numberOfContributions := funding.GetNumberOfContributionsBetween(now, nextRecurrence, timezone)
